...
static bool PrefilterIcmpv6mtuIsPrefilterable(const Signature *s)
{
    ...
    for (sm = s->init_data->smlists[DETECT_SM_LIST_MATCH] ; sm != NULL; sm = sm->next) {
        if (sm->type == DETECT_ICMPV6MTU) {
            DetectU32Data *du32 = (DetectU32Data *)sm->ctx;
            /* Mode "range" is not prefilterable
             * In order to become so, we should extend PrefilterPacketHeaderValue
             */
            return (du32->mode != DETECT_UINT_RA);
        }
    }
    return false;
}
